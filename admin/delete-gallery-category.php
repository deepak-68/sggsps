...
if (isset($_GET['id'])) {
    ...
    $delete_query = "DELETE FROM gallery_category WHERE category_id = '$category_id'";
    $delete_result = mysqli_query($db, $delete_query);
    
    if ($delete_result) {
        // Category successfully deleted
        header("location: manage-gallery-category.php?status=" . base64_encode(1)); // Redirect with success status
        exit(); // Terminate script execution after redirection
    } else {
        // Error occurred while deleting category
        header("location: manage-gallery-category.php?status=" . base64_encode(-1)); // Redirect with error status
        exit(); // Terminate script execution after redirection
    }
}
...
header("location: manage-gallery-category.php?status=" . base64_encode(-1));
exit(); // Terminate script execution after redirection
?>
