...
if (isset($_GET['id'])) {
    ...
    header("Location: manage-alumni-testimonial.php");
    exit(); // Terminate script execution after redirection
}

// Check if multiple testimonial IDs are provided for deletion
if (isset($_POST['alumni_ids'])) {
    $testimonial_ids = $_POST['alumni_ids'];
...
function deleteTestimonialImage($db, $test_id) {
    $image_query = "SELECT image FROM alumni WHERE alumni_id = '$test_id'";
    $image_result = mysqli_query($db, $image_query);
    $image_row = mysqli_fetch_assoc($image_result);
    $image_filename = $image_row['image'];
    $image_path = "alumnis/" . $image_filename;
...
function deleteTestimonial($db, $test_id) {
    $delete_query = "DELETE FROM alumni WHERE alumni_id = '$test_id'";
