...
$event_id = mysqli_real_escape_string($db, base64_decode($_GET['id']));
...
if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES["uploaded_file"]) && $_FILES["uploaded_file"]["size"] > 0) {
        ...
        if (!in_array($file_type, $allowed_types) && !$is_svg) {
            $msg = "<div class='alert alert-danger'>Please upload a valid image file.</div>";
        } elseif ($file_size > 2 * 1024 * 1024) {
            $msg = "<div class='alert alert-danger'>File size exceeds the 2MB limit.</div>";
        } else {
            $unique_filename = uniqid() . '_' . $original_name;
            if (move_uploaded_file($temp_name, $upload_directory . $unique_filename)) {
                $query = "UPDATE events SET image = '$unique_filename' WHERE event_id = $event_id";
                if (!mysqli_query($db, $query)) {
                    $msg = "<div class='alert alert-danger'>Failed to update image: " . mysqli_error($db) . "</div>";
                }
            } else {
    ...
    if (mysqli_query($db, $query)) {
        ...
        echo ("<script>window.location.href='manage-event.php?status=$status_code';</script>");
        exit;
    } else {
        $msg = "<div class='alert alert-danger'>Failed to update event: " . mysqli_error($db) . "</div>";
    }
}
                                    ...
                                    <div class="row">
                                        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                                            <div class="form-group">
                                                <label for="title" class="form-label">Events Title <span class="red-text">*</span></label>
                                        ...
                                        <div class="form-group">
                                            <label for="date" class="form-label">Date <span class="red-text">*</span></label>
                                            <input type="date" name="event_date" id="date" class="form-control" value="<?php echo $existingDate; ?>" required>
                                        </div>
                                    </div>
                                        ...
                                        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
    										<div class="form-group">
        										<label for="name" class="form-label">Image(724px400px) <span class="red-text">*</span></label>
                                        ...
                                        <!-- Modal end -->
                                        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                                            <div class="form-group"> Event Time <span class="red-text">*</span>
                                                <label class="sr-only control-label" for="name">Event Time<span
                                                        class=" ">
                                                    </span></label>
                                                	<input id="time" name="time" type="text" class="form-control input-md"
                                                    value="<?php echo $existingTime; ?>" 
                                                    required>
                                            </div>
                                        </div>
                                        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                                            <div class="form-group"> Event Location <span class="red-text">*</span>
                                                <label class="sr-only control-label" for="name">Event Location<span
                                                        class=" ">
                                                    </span></label>
                                                	<input id="location" name="location" type="text" class="form-control input-md"
                                                    value="<?php echo $existingLocation; ?>"
                                                    required>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                            <div class="form-group">
                                                <label for="details" class="form-label">Event Description <span class="red-text">*</span></label>
                                                <textarea class="form-control" rows="5" cols="45" 
                                                    name="description"><?php echo $existingDescription; ?></textarea>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                            <button type="submit" class="btn btn-secondary" name="submit"
                                                id="submit">
                                                <i class="feather icon-save"></i>&nbsp; Update Event
                                            </button>
                                        </div>
                                    </div>
                                </div>
