...
if (isset($_GET['id'])) {
    ...
    // Redirect to the manage-notice-board.php page
    header("Location: manage-notice-board.php");
...
// If no service ID provided, redirect to manage-notice-board.php
header("Location: manage-notice-board.php");
exit(); // Terminate script execution after redirection

// Function to delete event image file and record from the database
function deleteEvent($db, $event_id) {
    $sql = "SELECT image FROM notice_board WHERE notice_id = '$event_id'";
    ...
    $delete_sql = "DELETE FROM notice_board WHERE notice_id = '$event_id'";
