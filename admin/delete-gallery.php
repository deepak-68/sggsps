...
if (isset($_GET['id'])) {
    ...
    $delete_categories_sql = "DELETE FROM post_gallery WHERE gallery_id = '$post_id'";
    $delete_categories_result = mysqli_query($db, $delete_categories_sql);
    
    // Delete the post record from the database
    $delete_post_sql = "DELETE FROM gallery WHERE gallery_id = '$post_id'";
    ...
    if ($delete_categories_result && $delete_post_result) {
        ...
        header("Location: manage-gallery.php?status=" . base64_encode(1));
    } else {
        // Error in single deletion
        header("Location: manage-gallery.php?status=" . base64_encode(0));
    }
...
function deletePostImage($db, $post_id) {
    $image_query = "SELECT image FROM gallery WHERE gallery_id = '$post_id'";
    $image_result = mysqli_query($db, $image_query);
    $image_row = mysqli_fetch_assoc($image_result);
    $image_filename = $image_row['image'];
    $image_path = "services/" . $image_filename;
