...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT * FROM spotlight WHERE spotlight_id = $pageId";
    $result = mysqli_query($db, $query);
    
    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        // Fetch spotlight details from $row
        $pagetitle = $row['title'];
        $pageStatus = $row['status'];
        $pagecontent = $row['description'];
        $existingDate = $row['spotlight_date'];
        ...
        if (isset($_POST['submit'])) {
            ...
            $updatedpagecontent = mysqli_real_escape_string($db, $_POST['description']);
            $existingDate = $_POST['spotlight_date'];
            
            // Generate slug URL based on the updated page title
            // $slug = slugify($updatedpagetitle);
            
            // Update the spotlight in the database
            $updateQuery = "UPDATE spotlight SET title = '$updatedpagetitle', spotlight_date = '$existingDate',  status='$updatedpageStatus', description = '$updatedpagecontent' WHERE spotlight_id = $pageId";
            if ($db->query($updateQuery) === TRUE) {
                $status = 1;
                $re = base64_encode($status);
                echo ("<SCRIPT LANGUAGE='JavaScript'>window.location.href='manage-spotlight.php?status=$re';</SCRIPT>");
            }
        }
    } else {
        echo "Page not found!";
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h5 class="m-b-10">Update Spotlight</h5>
                            </div>
<!--                             <ul class="breadcrumb"> -->
                                    ...
                                    <div class="row">
                                        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
    										<div class="form-group">
        										<label for="title" class="form-label">Spotlight Title <span class="red-text">*</span></label>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="date" name="spotlight_date" id="date" class="form-control" value="<?php echo $existingDate; ?>" required>
                                        </div>
                                    </div>
                                        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                                            <div class="form-group">
                                        ...
                                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                            <div class="form-group">Description <span class="red-text">*</span>
                                                ...
                                                <textarea class="form-control" rows="5" cols="45" name="description"><?php echo $pagecontent; ?></textarea>
                                            </div>
                                        </div>
                                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                            <button type="submit" class="btn btn-secondary" name="submit" id="submit">
                                                <i class="feather icon-save"></i>&nbsp; Update Spotlight
                                           </button>
                                        </div>
                                    </div>
                                </div>
